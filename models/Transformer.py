...
class DecoderBlock(nn.Module):
    def __init__(self, dim, num_heads, dim_q = None, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here，这里也没用
...
class PCTransformer(nn.Module):
    ...
    def pos_encoding_sin_wave(self, coor):  #实验过程中用到的，最终没用
    ...
    def forward(self, inpc):
        ...
        # build point proxy
        bs = inpc.size(0)
        #用这个dgcnn主要是用于根据进行下采样，生成点代理，后续的计算在这个点代理上进行
        coor, f = self.grouper(inpc.transpose(1,2).contiguous()) #[48,2048,3]->([48,3,128],[48,128,128])一个是原始点的下采样，一个是变换后的点

        knn_index = get_knn_index(coor)#[48,3,128]->49152（48*8*128）找每个点的最近的8个点的索引，值分别是0，128，256，。。。6144（128*48）
        # NOTE: try to use a sin wave  coor B 3 N, change the pos_embed input dim
        # pos = self.pos_encoding_sin_wave(coor).transpose(1,2)
        # [48,3,128]->[48,384,128]->[48,128,384],transformer的隐藏层大小为384，相当于做了一个类似于词嵌入的操作，为什么是384呢？3*128
        # 输入的是下采样之后的原始点，输出的是一个384维的嵌入
        pos =  self.pos_embed(coor).transpose(1,2)
        x = self.input_proj(f).transpose(1,2)  #[48，128，128]->[48,128,384],对变换后的点也计算嵌入
        ...
        for i, blk in enumerate(self.encoder):
            if i < self.knn_layer: #这个参数这里设置为1，也就是只有第一个头做了一些不一样的操作
                x = blk(x + pos, knn_index)   # B N C  x:[48,128,384]->[48,128,384]
            else:
                x = blk(x + pos)
        # build the query feature for decoder
        # global_feature  = x[:, 0] # B C

        global_feature = self.increase_dim(x.transpose(1,2)) # B 1024 N [48,128,384]->[48,1024,128] 1维卷积升维
        global_feature = torch.max(global_feature, dim=-1)[0] # B 1024  得到一个点云的一个1维向量作为全局表示

        coarse_point_cloud = self.coarse_pred(global_feature).reshape(bs, -1, 3)  #  B M C(3)  （[48,1024]->[48,384]->[48,96,3])这个96是shapenet粗点云的点的数量，可以算骨干点吧

        new_knn_index = get_knn_index(coarse_point_cloud.transpose(1, 2).contiguous())  #[36864],48*96*8，96个点的8个最近邻
        # ([48,3,128],[48,3,96])->[36864],96个粗点和原始128个点中的8个最近邻
        cross_knn_index = get_knn_index(coor_k=coor, coor_q=coarse_point_cloud.transpose(1, 2).contiguous())

        query_feature = torch.cat([  #将原始点云的全局向量与粗点云向量拼接
            global_feature.unsqueeze(1).expand(-1, self.num_query, -1), #复制96份，然后和粗点云拼接拼接[48,1024]->[48,96,1024]
            coarse_point_cloud], dim=-1) # B M C+3 [48,96,1027]
        q = self.mlp_query(query_feature.transpose(1,2)).transpose(1,2) # B M C [48,96,1027]->[48,96,384]，映射到384好让transformer解码
        # decoder
        for i, blk in enumerate(self.decoder):#[48,96,384]->[48,96,384]
            if i < self.knn_layer:
                q = blk(q, x, new_knn_index, cross_knn_index)   # B M C
            else:
                q = blk(q, x)

        return q, coarse_point_cloud#[48,96,384],[48,96,3]
