...
class Metrics(object):
    ITEMS = [{
        ...
        'is_greater_better': False,  #指定比较方式，false表示越小越好
        'init_value': 32767
    }, {
        'name': 'Pre_Gt',
        'enabled': True,  #可以通过这个属性来开关相应的metrices
        'eval_func': 'cls._Pre_Gt',
        'is_greater_better': False,  #指定比较方式，false表示越小越好
        'init_value': 32767
    }, {
        'name': 'Gt_Pre',
        'enabled': True,
        'eval_func': 'cls._Gt_Pre',
        'is_greater_better': False,  #指定比较方式，false表示越小越好
        'init_value': 32767
    }]
    ...
    @classmethod
    def _Gt_Pre(cls,pred, gt):
        return cls.ITEMS[4]['init_value']

    @classmethod
    def _Pre_Gt(cls,pred, gt):
        return cls.ITEMS[3]['init_value']
    ...
    def _get_chamfer_distancel1(cls, pred, gt):
        ...
        return chamfer_distance(pred, gt).item() #* 1000  #PFNet和DGNet再CD距离的计算函数内乘系数，统计的时候这里不再乘

    @classmethod
    def _get_chamfer_distancel2(cls, pred, gt):
        chamfer_distance = cls.ITEMS[2]['eval_object']
        return chamfer_distance(pred,gt).item()*1000
        # chamfer_distance = cls.ITEMS[2]['eval_object']   #PFNet
        # dist, Pre_Gt, Gt_Pre=chamfer_distance(pred, gt)
        # cls.ITEMS[3]['init_value'] = Pre_Gt.item()
        # cls.ITEMS[4]['init_value'] = Gt_Pre.item()
        # return dist.item() #* 1000  #PFNet和DGNet在CD距离的计算函数内乘系数，统计的时候这里不再乘
    ...
    def better_than_scale(self, other):
        if other is None:
            return True
        ...
        _value = self._values[_index]
        other_value = other._values[_index]
        return _value > other_value*0.95 if _metric['is_greater_better'] else _value*0.95 < other_value  #other是最优值，乘一点系数让最优的范围放宽一点，
